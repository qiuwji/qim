...
func (a *SessionActor) handleChangePassword(ctx actor.Context, msg ChangePasswordCmd) {
	...
	if u.Password != msg.OldPassword {
		ctx.Reply(Result{Err: fmt.Errorf("incorrect old password")})
		return
	}
	now := time.Now().Unix()
	if err := a.store.UpdateUser(a.uid, map[string]any{"password": msg.NewPassword, "updated_at": now}); err != nil {
		ctx.Reply(Result{Err: err})
		return
	}
	ctx.Reply(Result{Data: true})
