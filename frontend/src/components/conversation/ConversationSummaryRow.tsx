...
export function ConversationSummaryRow({ conversation, detail, subtitle, members, currentUID, userCache, onlineMap, mentionMap, selected = false, onClick }: {
  ...
  const hasMention = mentionMap?.[conversation.conversation_id] && conversation.unread_count > 0;
  ...
  return (
    <button className={`flex w-full items-center gap-3 border-b border-[#f0f1f3] px-3 py-2.5 text-left transition hover:bg-[#e8e8e8] ${selected ? 'bg-[#d4edda]' : ''}`} onClick={() => onClick(conversation.conversation_id)}>
      ...
      <div className="min-w-0 flex-1">
        ...
        {subtitle && <div className="text-xs text-[#999]">{hasMention ? <span className="mention-badge">[有人@我]</span> : null}{subtitle}</div>}
      </div>
