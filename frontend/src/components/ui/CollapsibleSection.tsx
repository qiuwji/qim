...
export function CollapsibleSection({ title, count, open, onToggle, extraActions, small, children }: {
  ...
  return (
    <div className="border-b border-[#e4e7ec]">
      <button className={`flex w-full items-center gap-1.5 rounded-md px-2 text-left text-[#555f6d] hover:bg-[#eef1f5] ${small ? 'py-1 text-xs' : 'py-2 text-sm'}`} onClick={onToggle}>
        <span className={`text-[10px] transition-transform ${open ? 'rotate-90' : ''}`}>▸</span>
        <strong className={`font-semibold ${small ? 'text-xs' : 'text-sm'}`}>{title}</strong>
        <span className={`${small ? 'text-[10px]' : 'text-xs'} text-[#9aa1ad]`}>{count}</span>
