...
export function ContactsPanel({ keyword, setKeyword, onSearch, results, friends, friendGroups, requests, outgoingReqs, groupConversations, details, userCache, friendMap, onStartPrivate, onRequest, onHandleRequest, onSelectChat, onDeleteFriend, onUpdateRemark, onCreateGroup, onRenameGroup, onDeleteGroup, onViewUser }: {
  keyword: string; setKeyword: (v: string) => void; onSearch: () => void; results: UserDTO[]; friends: FriendDTO[]; friendGroups: FriendGroupDTO[]; requests: FriendRequestDTO[]; outgoingReqs: FriendRequestDTO[]; groupConversations: UserConvDTO[]; details: Record<number, ConversationDTO>; userCache: Record<number, UserDTO>; friendMap?: Record<number, FriendDTO>;
  ...
  return (
    <div className="contacts-panel">
      ...
      {results.length > 0 && <CollapsibleSection title="搜索结果" count={results.length} open={openSections.search ?? true} onToggle={() => toggle('search')}>{results.map((item) => { const isFriend = !!friendMap?.[item.id]; return (<UserLine key={item.id} user={item} actions={<>{isFriend && <button onClick={() => onStartPrivate(item.id)}>聊天</button>}{!isFriend && <button onClick={() => void onRequest(item.id)}>加好友</button>}</>} onViewUser={onViewUser} />); })}</CollapsibleSection>}
