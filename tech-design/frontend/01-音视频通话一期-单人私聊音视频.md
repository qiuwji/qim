...
const [busyInfo, setBusyInfo] = useState<string | null>(null);
...
msg.type === 'call' && msg.action === 'accepted'
  → useCallStore.setState('connected')  → 创建 offer
...
- WS 断开（`system/closed`）时如果 `callState === 'connected'` → 自动 `cleanupCall` + 提示"连接已断开"
